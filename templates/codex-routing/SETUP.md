...
安装、回退和冲突保存期间须保持受管理文件、父目录和事务备份证据稳定，避免其他写入者；无关的 Codex 会话和缓存活动可继续。工具不加独占锁，身份和摘要检查与随后的替换或删除不是原子条件操作，最后窗口内的新修改仍可能被覆盖或删除。无法安排这些目标的独占访问时先不要执行。
...
有文件变化时保留安装器打印的事务清单；没有变化时只复核文件，不创建空备份。回退仅针对所选事务，在检查发现文件后续修改时拒绝覆盖。若安装失败且自动恢复不完整，保留的清单和备份用于逐文件核对，`recovery-required.json` 记录失败状态；先确认当前文件摘要，再决定修复，不能直接重跑整个事务。单个文件替换是原子的，整个目录更新不具备同样保证。
...
路由测试还要按 [README](README.md#source-verification) 的 PowerShell 命令在 Windows 原生 Python 中运行。测试需要 PATH 中可用的 Git；平台专属检查和缺少符号链接权限的用例会明确标为跳过，不能把 WSL 测试通过当作 Windows 原生验证。
