...
Do not delegate trivial questions, explanations, single-file changes, single-command checks, or work whose scope is not yet clear. The global child-agent concurrency cap in WSL is **1**; an existing trusted project overlay may set it to **2**. Follow the effective configured cap. Never run overlapping writers or allow two roles to modify the same files or repository state.
