...
The Windows environment owns Windows-native applications, GUI tooling, and desktop integration. WSL owns Linux development, data processing, GPU and remote-training orchestration. Keep Windows and WSL duties independent; do not copy login/provider or native-tool settings across them.

Do not delegate trivial questions, explanations, single-file changes, single-command checks, or work whose scope is not yet clear. The user-level Windows child-agent concurrency limit is **2 per primary session**, excluding the primary; it is not a machine-wide pool shared across sessions. Maintain this setting globally, without project overrides. This is a ceiling, not a requirement to spawn two agents. When useful, one slot may supervise an authorized job through read-only `scout` while the other handles an independent bounded implementation, exploration, or review task. Never run overlapping writers or allow two roles to modify the same files or repository state; keep one writer per worktree, including the primary.
...
When dispatching a child agent, tell the user its returned nickname, role, configured model, and reasoning effort in a concise commentary update (for example, `nickname — role — model — effort`). Read the live role binding instead of duplicating a model table here. If the spawn result or runtime does not expose the resolved model or effort, label them as configured values rather than claiming runtime verification; do not imply this instruction changes the app's automatic agent-start notification.
...
When one authorized long-running job has a live `scout`, let that agent own routine read-only progress and resource checks. The primary should not duplicate frequent polling; it checks original evidence at planned checkpoints, stage transitions, anomalies, completion, or when `scout` coverage ends. Ask `scout` to send concise updates and urgent alerts proactively through the available agent messaging mechanism. If the runtime cannot maintain or deliver that supervision, the agent must hand back pending checks and the primary resumes them. The primary still starts authorized jobs and independently verifies final evidence and acceptance.
