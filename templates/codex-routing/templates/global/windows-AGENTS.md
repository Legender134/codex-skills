...
Windows owns native applications, GUI tooling, and desktop integration. WSL owns Linux development, Git, tests, builds, data processing, and remote-job orchestration. Use the environment responsible for the task; keep credentials, providers, and native tooling separate.

The user-level Windows child-agent limit is **2 per primary session**, excluding the primary. Maintain it globally without project overrides. This is a ceiling, not a target or a machine-wide pool; live tool limits also apply.
