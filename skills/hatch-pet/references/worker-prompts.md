...
For a substantial image-heavy run, bounded isolated workers can keep each visual job focused. Use delegation only when available, authorized, and useful; do not assume that a smaller model or extra agent is cheaper overall. Preserve the full v2 quality contract.

## Subagent Delegation

Follow the active routing policy and live tool contract. Routine deterministic checks and small repairs stay with the primary. Use independent workers for full-run visual jobs and unbiased QA when that separation is needed; run them sequentially when capacity is one.

Parent responsibilities:

- complete brand discovery before preparation for a bare brand/product/company/prospect name; do it directly when a separate worker adds overhead
